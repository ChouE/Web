...
| Mac快捷键 | Win快捷键 | 作用 | 备注 |
|:-------------|:-------------|:-----|:-----|
| **Cmd + ←** | **HOME**  | 将光标定位到当前行的最左侧 | 很常用 |
| **Cmd + →**|  **END**   | 将光标定位到当前行的最右侧 | 很常用 |
...
- 「Cmd + 左右方向键」：在**整行**之间移动光标（很常用）。Windows中为HOME&END
...
**技巧3**：在某个位置按住「Alt + Shift」键，可以向下在同一位置拉取光标，适合批量更改同种格式的复制粘贴型语句

**技巧4**：选中一堆文本后，按住「Option + Shift + i」键（windows 用户是按住「Alt + Shift + I」键），既可在**每一行的末尾**都创建一个光标。
...
> 目前版本的VScode已经内置了这个选项，不用再进行设置，可以忽略此步骤
...
当我们尝试去修改某个函数（或者变量名）时，我们可以把光标放在上面，然后按下「Ctrl + F2」键，那么，这个函数（或者变量名）出现的地方都会被修改。
